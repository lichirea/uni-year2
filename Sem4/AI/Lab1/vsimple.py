...
class Drone():
    ...
    def moveDSF(self, m, alreadyvisited, e, deadends):


        stack = []
        m.markDetectedWalls(e, self.x, self.y)
        if self.x > 0:
            if m.surface[self.x - 1][self.y] == 0 and (self.x - 1, self.y) not in alreadyvisited:
                self.i = -2
        ...
        if self.x < 19:
            if m.surface[self.x + 1][self.y] == 0 and (self.x + 1, self.y) not in alreadyvisited:
                ...
                stack.append((self.x + 1, self.y))
        if self.y > 0:
            if m.surface[self.x][self.y - 1] == 0 and (self.x, self.y - 1) not in alreadyvisited:
                self.i = -2
        ...
        if not stack:
            if self.x > 0:
                if m.surface[self.x - 1][self.y] == 0  and (
                        self.x - 1, self.y) not in deadends:
                    stack.append((self.x - 1, self.y))
            if self.x < 19:
                if m.surface[self.x + 1][self.y] == 0  and (
                        self.x + 1, self.y) not in deadends:
                    stack.append((self.x + 1, self.y))
            if self.y > 0:
                if m.surface[self.x][self.y - 1] == 0  and (
                        self.x, self.y - 1) not in deadends:
                    stack.append((self.x, self.y - 1))
            if self.y < 19:
                if m.surface[self.x][self.y + 1] == 0  and (
                        self.x, self.y + 1) not in deadends:
                    stack.append((self.x, self.y + 1))
            if (self.x, self.y) not in deadends:
                deadends.append((self.x, self.y))

        if not stack:
            moveTo = alreadyvisited[self.i]
            alreadyvisited.append(moveTo)
            self.i -= 2
        else:
            moveTo = stack.pop()
            alreadyvisited.append(moveTo)
...
def main():
    ...
    while running:
        ...
        if result:
            result = d.moveDSF(m, alreadyvisited, e, deadends)
